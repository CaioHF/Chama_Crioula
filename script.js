...
function formatarPeso(qtd, isUnit) {
  if (isUnit) return `${Math.round(qtd)} un`;
  if (qtd < 1) {
    const gramas = Math.round(qtd * 1000);
    return `${gramas}g`;
  } else {
    if (qtd % 1 === 0) {
      return `${qtd.toFixed(2).replace('.', ',')}kg`;
    } else {
  ...
  lista.forEach((p) => {
    ...
    produtosContainer.innerHTML += `
      <article class="produto" data-index="${index}">
        ...
        <div class="info" onclick="abrirModalProduto(${index})" style="cursor: pointer;">
          <h3>${p.nome}</h3>
          <div class="preco-box">${htmlPreco}</div>
          <button class="btn-add-prod btn-selecionar-prod" onclick="event.stopPropagation(); abrirModalProduto(${index});">Selecionar</button>
        </div>
      </article>
    `;
  });
...
function atualizarCarrinho(){
    ...
    carrinho.forEach((item, i) => {
      ...
      html += `
        <div class="cart-item" data-key="${item.key}">
          <img src="${item.img || 'imagens/default.png'}" class="cart-item-thumb" alt="${item.nome}">
          <div class="cart-left">
            <div><strong>${item.nome}</strong></div>
            ${item.corte ? `<div class="cart-item-corte">Corte: ${item.corte}</div>` : ''}
            ${item.obs ? `<div class="cart-item-obs">Obs: ${item.obs}</div>` : ''}
            <div style="font-size: 0.85rem; margin-top: 3px;">Qtd: ${formatarPeso(item.qtd, isUnit)} — R$ ${subtotal.toFixed(2)}</div>
          </div>
          <div class="cart-controls">
            <button class="cart-minus" data-i="${i}">-</button>
            <button class="cart-plus" data-i="${i}">+</button>
          </div>
          <button class="cart-remove-item" data-i="${i}" title="Excluir produto da lista">
  ...
  const cartCountFloatEl = document.getElementById('cart-count-float');
  const btnCartFloat = document.getElementById('btn-carrinho-flutuante');
  ...
  if(cartCountFloatEl) {
      cartCountFloatEl.textContent = qtdItens;
  }
  if(btnCartFloat) {
      btnCartFloat.style.display = qtdItens > 0 ? 'flex' : 'none';
  }
  ...
  produtosContainer.addEventListener('click', (ev) => {
    ...
    if(btn.dataset.action === 'add'){
      ...
      if(existing){
        ...
        carrinho.push({
          key, nome: p.nome, preco: p.preco, categoria: p.categoria, corte: corte, qtd: qty, img: p.img
        });
      }
...
document.addEventListener('click', (ev) => {
  const btnRemove = ev.target.closest('.cart-remove-item');
  if(btnRemove) {
    const idx = Number(btnRemove.dataset.i);
...
// =========================================
function enviarPedidoWhatsApp(idRua, idNumero, idBairroSelect, idPagamento, idData, idHora) {
...
if(btnFinalizar) {
    btnFinalizar.addEventListener('click', () => {
        enviarPedidoWhatsApp('rua-desk', 'numero-desk', 'bairro-desk', 'pagamento-desk', 'data-entrega-desk', 'horario-entrega-desk');
    });
}
...
if(btnFinalizarMobile) {
    btnFinalizarMobile.addEventListener('click', () => {
        enviarPedidoWhatsApp('rua', 'numero', 'bairro', 'pagamento', 'data-entrega-mobile', 'horario-entrega-mobile');
    });
}
// =========================================
// 8. MODAIS
...
if(btnCartFloat && sidebar){
  btnCartFloat.addEventListener("click", (e) => { 
      e.stopPropagation(); 
      sidebar.classList.add("show"); 
  });
}
...
document.addEventListener('click', (event) => {
    ...
    if (sidebar && sidebar.classList.contains('show')) {
        ...
        if (!sidebar.contains(event.target) && 
            (!btnCartMobile || !btnCartMobile.contains(event.target)) &&
            (!btnCartFloat || !btnCartFloat.contains(event.target))) {
            fecharCarrinho();
        }
    }
});
...
function abrirModalProduto(index) {
    ...
    const inputObs = document.getElementById('detalhe-obs');
...
function atualizarInputModal(isUnit) {
    ...
    if(input) {
        input.value = formatarPeso(qtdAtualModal, isUnit);
    }
    atualizarBotaoPreco();
}
...
// Digitação manual da quantidade
(function() {
    const inputQtd = document.getElementById('detalhe-qtd');
    if (!inputQtd) return;
    inputQtd.removeAttribute('readonly');
    inputQtd.style.cursor = 'text';

    inputQtd.addEventListener('focus', () => {
        inputQtd.select();
    });
    ...
    inputQtd.addEventListener('blur', () => {
        ...
        const p = produtos[produtoSelecionadoIndex];
        const isUnit = categoriasUnidade.includes(p.categoria);
        // Pega texto digitado, trata vírgula e remove unidades
        let raw = inputQtd.value.replace(',', '.').replace(/[^0-9.]/g, '');
        let val = parseFloat(raw);
        if (isNaN(val) || val <= 0) val = isUnit ? 1 : 0.5;
        const minVal = isUnit ? 1 : 0.5;
        val = Math.max(minVal, val);
        if (!isUnit) val = Math.round(val * 100) / 100;
...
document.getElementById('btn-adicionar-modal').onclick = () => {
    ...
    const inputObs = document.getElementById('detalhe-obs');
    const obsEscolhida = inputObs ? inputObs.value.trim() : '';

    const key = `${p.nome}|${corteEscolhido || ''}|${obsEscolhida || ''}`;
    const existing = carrinho.find(i => i.key === key);
    if(existing){ existing.qtd = +(existing.qtd + qtdAtualModal).toFixed(2); } 
    else { carrinho.push({ key, nome: p.nome, preco: p.preco, categoria: p.categoria, corte: corteEscolhido, obs: obsEscolhida, qtd: qtdAtualModal, img: p.img }); }
    atualizarCarrinho();
...
// =========================================
// LÓGICA DO MODAL DE CONFIGURAÇÕES E MODO NOTURNO
// =========================================
(function() {
    ...
    function aplicarTema(modoNoturno) {
        ...
        try { localStorage.setItem('modoNoturnoChamaCrioula', modoNoturno ? 'true' : 'false'); } catch(e){}
    }
    ...
    if (btnFecharConfig && modalConfig) {
